...
## ⚠️ 当前分支：`highauto`（公网付费版）

**`main` 分支为 8083 内测版，所有功能免费不限量开放。本分支已部署到 8085 端口。**

### 两个分支的定位

| 分支 | 定位 | 端口 | 当前状态 |
|------|------|------|---------|
| `main` | 内测版 | 8083 | 线上运行，不限量免费使用 |
| `highauto` | 公网付费版 | 8085 | 线上运行，付费解锁 |
...
- `components/AppShell.tsx` — 集成付费墙，未解锁时限制打磨和预审次数
- `components/LandingPage.tsx` — 去个人化，保留轻量付费提示条
