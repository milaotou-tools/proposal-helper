...
export function ProposalHelperApp() {
  ...
  function generateFramework() {
    ...
    void runAction("从课题想法生成申报书框架", () => {
      ...
      return postAi("/api/generate-framework", payload, allowCollection);
    });
  }
          ...
          <div className="rounded-md border border-[#E8E6E1] bg-white p-5">
            {mode === "framework" ? (
              <FrameworkPanel
                ...
                onAllowCollectionChange={setAllowCollection}
              />
            ) : (
              <DraftPanel
                draft={draft}
                ...
                onReview={() =>
                  runAction("申报书草稿问题诊断", () => {
                    ...
                    return postAi("/api/review-draft", payload, allowCollection);
                  })
                }
                onPolish={() =>
                  runAction("申报书逐栏打磨", () => {
                    const payload = { draft, section: scope };
                    return postAi("/api/polish-section", payload, allowCollection);
                  })
                }
                onExpertReview={() =>
                  runAction("模拟专家预审", () => {
                    const payload = { draft };
                    return postAi("/api/expert-review", payload, allowCollection);
                  })
                }
...
function FrameworkPanel({
  ...
  onAllowCollectionChange: (v: boolean) => void;
}) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <DataCollectionCheckbox checked={allowCollection} onChange={onAllowCollectionChange} />
